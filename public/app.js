document.addEventListener("DOMContentLoaded", () => {
  ...
  const camBtn = $("#camBtn");
  const screenBtn = $("#screenBtn"); // "Presentar"
  ...
  async function startCamera(preferBack = false) {
    ...
    // En móvil, a veces falla "environment". Hacemos fallback.
  ...
  async function startPresentSmart() {
    // Si NO es contexto seguro, en móvil va a fallar permisos y parece "no hace nada"
    ...
    // Móvil / iOS / no screen share => cámara trasera
    ...
    // Desktop: screen share, si falla fallback a cámara
    try {
    ...
    listenBtn.onclick = async () => {
      try {
        ...
        listenBtn.onclick = () => {
          vid.muted = true;
          listenBtn.textContent = "🔊 Escuchar";
          listenBtn.onclick = async () => { try { vid.muted = false; await vid.play(); } catch {} };
        };
      } catch {
  ...
  camBtn && (camBtn.onclick = () => startCamera(false).catch((e) => setStatus(`❌ Cámara: ${e?.name || "error"}`)));
  screenBtn && (screenBtn.onclick = () => startPresentSmart().catch((e) => setStatus(`❌ Presentar: ${e?.name || "error"}`)));
